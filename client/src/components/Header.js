...
const countryData = ["Âu Mỹ", "Hàn Quốc", "Trung Quốc"];
...
const Header = () => {
  ...
  return (
    <div className="sticky top-0 left-0 right-0 py-1 px-3 md:px-0 md:py-4 bg-[#1c1c1e] z-10 md:flex md:justify-between items-center border-b-[1px] border-[rgba(255,255,255,0.17)] md:pl-5 lg:pl-0">
      <div className="hidden md:flex justify-between gap-3">
        <Link to="/filter?type=single" className="tag-primary">
          Phim Lẻ
        </Link>
        <Link to="/filter?type=series" className="tag-primary">
          Phim Bộ
        </Link>
      ...
      {!username && (
        <Link
          to="/login"
          className="hidden lg:flex lg:items-center lg:gap-2 bg-neutral-800 p-2 cursor-pointer hover:bg-red-500 transition duration-300 ease-in-out rounded-md "
        >
          <AiOutlineUser />
          <span>Đăng nhập</span>
        </Link>
      ...
      <div
        className={`${
          isOpenMenu ? "" : "hidden"
        } md:hidden fixed left-0 bottom-0 h-screen bg-[#15151577] w-full`}
      >
        <div
          className={`${
            isOpenMenu && "open"
          } w-[68%] h-screen bg-[#151515] pt-20 pl-5`}
          ref={menuRef}
        >
          <Sidebar />
        </div>
